// RaccoonSkunkDefenseInteraction.cs (최적화된 버전)
...
public class RaccoonSkunkDefenseInteraction : BasePetInteraction
{
    ...
    [Header("접근 설정")]
    [Tooltip("너구리가 스컹크에게 접근할 거리")]
    public float approachDistance = 3f;

    [Tooltip("너구리가 접근하는 속도 배율")]
    public float raccoonApproachSpeedMultiplier = 1.2f;
    ...
    [Tooltip("너구리가 물러나는 거리")]
    public float raccoonRetreatDistance = 10f;

    [Tooltip("너구리가 물러나는 속도 배율")]
    public float raccoonRetreatSpeedMultiplier = 2.0f;
    ...
    private IEnumerator ApproachPhase(PetController attacker, PetController skunk)
    {
        ...
        attacker.agent.speed = attacker.baseSpeed * raccoonApproachSpeedMultiplier;
 ...
 // 파티클 효과 생성
    ...
    private IEnumerator AttackerRetreatPhase(PetController attacker, PetController skunk)
    {
        ...
        Vector3 retreatTarget = attacker.transform.position + retreatDirection * raccoonRetreatDistance;
        retreatTarget = FindValidPositionOnNavMesh(retreatTarget, raccoonRetreatDistance * 1.5f);

        // 어지러워하며 비틀거리며 후퇴
        attacker.agent.isStopped = false;
        attacker.agent.speed = attacker.baseSpeed * raccoonRetreatSpeedMultiplier;
